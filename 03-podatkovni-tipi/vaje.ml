...
let dollar_to_euro (Dollar dollar) = Euro (dollar *. 0.86)

let euro_to_dollar (Euro euro) = Dollar (euro *. 1.5)
...
let rec intbool_separate ib_list =
  let rec pomozna int_acc bool_acc ib_list = 
    ...
    | Int (i, ibs) -> pomozna (i :: int_acc) bool_acc ibs
    | Bool (b, ibs) -> pomozna int_acc (b :: bool_acc) ibs
  in
  pomozna [] [] ib_list

  let rec intbool_separate2 ib_list =
  let rec pomozna int_acc bool_acc ib_list = 
    match ib_list with
...
(*let update {fire = fi; frost = fr; arcane = ar} magic =
  (*ali:   let {fire = fi; frost = fr; arcane = ar} = counter in *)
  match magic with
  | Fire -> {fire = fi +1; frost = fr; arcane = ar}
  | Frost -> {fire = fi +1; frost = fr; arcane = ar}
  | Arcane -> {fire = fi +1; frost = fr; arcane = ar}*)
