...
[*----------------------------------------------------------------------------*)
let rec collatz x =
  let rec aux trenutni  acc =
  match trenutni with
  | 1 -> List.rev (1 :: acc)
  | x when x mod 2 = 0 -> aux (x/2) (x :: acc)
  | x -> aux (3 * x + 1) (x :: acc)
  in
  if x <= 0 then [] else aux x [] 
...
let fiksne_tocke f sez =
  let rec aux acc =
    function
    | x :: xs -> if f x = x then  aux (x :: acc) xs else aux acc xs
    | [] -> acc
  in
  aux [] sez
...
let partition n sez =
  ...
  | x :: xs -> if List.length acc < n then aux (x :: acc) xs else List.rev acc :: aux [] (x :: xs)
  in
  aux [] sez
...
let kompozitum p q = List.map (fun x -> List.nth p x) q
...
let inverz p =
  let pari = List.mapi (fun i x -> (x, i)) p 
  in
  let aux = List.sort (fun (x, i) (y, j) -> compare x y) pari 
  in
  List.map snd aux
...
let cikli p =
  let pari = List.mapi (fun x i -> (x, i)) p 
  in
  let index = List.map (fun x -> x) (List.map snd pari) 
  in
  let rec najdi_cikel zacetni trenutni neobdelan cikel =
    let cikel' = trenutni:: cikel 
    in
    let neobdelan' = List.filter (fun x -> x != trenutni) neobdelan 
    in
    let naslednik = List.nth p trenutni 
    in
    if naslednik = zacetni then (cikel', neobdelan') else 
      najdi_cikel zacetni naslednik neobdelan' cikel'
      in
      let rec aux neobdelan acc =
        match neobdelan with
        | [] -> List.rev acc
        | x :: xs ->
          let (cikel, novi_neobdelani) = najdi_cikel x  x xs [] 
          in
          aux novi_neobdelani (cikel :: acc)
      in
      aux index []
...
let iz_transpozicij n sez_t =
  let rec zacetna i = if i >= n then [] else i :: zacetna (i + 1)
  in
  let zacetek = zacetna 0 
  in
  let rec aux transpozicije acc =
    match transpozicije with
    | [] -> acc
    | (x, y) :: xs ->
        let permutacija = List.mapi (fun i element -> if i = x then List.nth acc y else if i = y then List.nth acc x else element) acc 
        in
        aux xs permutacija
  in
  aux sez_t zacetek
...
let v_transpozicije permutacija =
  let cikli = cikli permutacija 
  in
  let rec razcleni_cikel cikel acc =
    match cikel with
    | [] -> acc
    | x :: xs ->
        let rec aux xs acc2 =
          match xs with
          | [] -> acc2
          | y :: ys -> aux ys ((x, y) :: acc2)
        in
        aux xs acc
  in
  let rec aux2 cikl acc =
    match cikl with
    | [] -> List.rev acc
    | c :: cs ->
        let acc' = razcleni_cikel c acc 
        in
        aux2 cs acc'
  in
  aux2 cikli []
...
let dodaj i j n m =
  let visina = Array.length m in
  let rec vrstice indeks =
    if indeks >= visina then [||] else let vrstica = m.(indeks) 
      in
      let nova_vrstica =
        if indeks = i then let sirina = Array.length vrstica 
          in
          let rec sestavi_stolpec stolpec_i =
            if stolpec_i >= sirina then [||] else 
              let nova_vrednost = 
              if stolpec_i = j then (Some n) else vrstica.(stolpec_i) 
              in
              let ostali_stolpci = sestavi_stolpec (stolpec_i + 1) 
              in
              let novi_stolpec = Array.make 1 nova_vrednost 
              in
              Array.append novi_stolpec ostali_stolpci
          in
          sestavi_stolpec 0
        else
          vrstica
      in
      let ostale_vrstice = vrstice (indeks + 1) in
      Array.append [| nova_vrstica |] ostale_vrstice
  in
  vrstice 0
...
let izpis_mreze mreza =
  let rec ena_vrstica vrstica indeks acc =
    if indeks >= Array.length vrstica then
      "| " ^ acc ^ " |"
    else
      let element = 
        match vrstica.(indeks) with 
        | Some n -> string_of_int n
        | None -> "." 
      in
      let locilo =
      if indeks = 8 then "" else if indeks mod 3 = 2 then " | "  else " "
  in 
  ena_vrstica vrstica (indeks + 1) (acc ^ element ^ locilo)
  in
  let rec vse_vrstice mreza indeks acc =
    if indeks >= Array.length mreza then
      String.concat "\n" (List.rev acc)
    else
      let vrstica' = ena_vrstica mreza.(indeks) 0 "" in
      let acc2 = vrstica' :: acc in
      let koncni_acc =
        if indeks mod 3 = 2 && indeks < 8 then "+-------+-------+-------+" :: acc2 else acc2
      in
      vse_vrstice mreza (indeks + 1) koncni_acc
  in
  vse_vrstice mreza 0 ["+-------+-------+-------+"]
...
let izpis_resitve resitev =
  let rec ena_vrstica vrstica indeks acc =
    if indeks >= Array.length vrstica then
      ...
      let element = string_of_int vrstica.(indeks) 
      in
      let locilo = if indeks = 8 then "" else if indeks mod 3 = 2 then " | "  else " "
      in
      ena_vrstica vrstica (indeks + 1) (acc ^ element ^ locilo)
  in
  let rec vse_vrstice resitev indeks acc2 =
    if indeks >= Array.length resitev then
      String.concat "\n" (List.rev acc2)
    else
      let vrstica' = ena_vrstica resitev.(indeks) 0 "" 
      in
      let nov_acc = vrstica' :: acc2 in
      let koncni_acc =
        if indeks mod 3 = 2 && indeks <= 8 then 
          "+-------+-------+-------+" :: nov_acc else nov_acc
      in
      vse_vrstice resitev (indeks + 1) koncni_acc
  in
  vse_vrstice resitev 0 ["+-------+-------+-------+"]
...
let ustreza mreza resitev =
  let visina = Array.length mreza 
  in
  let sirina = Array.length mreza.(0) 
  in
  let rec preveri i j =
    if i >= visina then true else if j >= sirina then preveri (i + 1) 0 else
      match mreza.(i).(j) with
      | Some n -> if resitev.(i).(j) = n then preveri i (j + 1) else false
      | None -> preveri i (j + 1)
  in
  preveri 0 0
...
(*----------------------------------------------------------------------------*
 ...
 ```
[*----------------------------------------------------------------------------*)
let ni_v_vrstici (mreza, indeks) n  =
let visina = Array.length mreza
in
let sirina = Array.length mreza.(0)
in
let rec preveri_celico indeks j n =
  if indeks >= visina then false else if j >= sirina then true else 
    match mreza.(indeks).(j) with
    | Some x -> if x = n then false else preveri_celico indeks (j + 1) n
    | None -> preveri_celico indeks (j + 1) n
in
preveri_celico indeks 0 n
...
let ni_v_stolpcu (mreza, indeks) n  =
let visina = Array.length mreza
in
let sirina = Array.length mreza.(0)
in
let rec preveri_celico i indeks n =
  if i >= visina then true else if indeks >= sirina then false else 
    match mreza.(i).(indeks) with
    | Some x -> if x = n then false else preveri_celico (i + 1) indeks n
    | None -> preveri_celico (i + 1) indeks n
in
preveri_celico 0 indeks n

let ni_v_skatli (mreza, indeks_skatle) n  =
  let zacetna_vrstica = 
    match indeks_skatle / 3 with
    | 0 -> 0
    | 1 -> 3
    | 2 -> 6
    | _ -> failwith "Neveljaven indeks"
  in
  let zacetni_stolpec = 
    match indeks_skatle mod 3 with
    | 0 -> 0
    | 1 -> 3
    | 2 -> 6
    | _ -> failwith "Neveljaven indeks"
  in
  let rec preveri_celico indeks =
    if indeks > 8 then true else
      let i = zacetna_vrstica + (indeks / 3) 
      in
      let j = zacetni_stolpec + (indeks mod 3) 
      in
      match mreza.(i).(j) with
      | Some x -> if x = n then false else preveri_celico (indeks + 1)
      | None -> preveri_celico (indeks + 1)
  in
  preveri_celico 0
...
let kandidati mreza i j =
  let indeks_skatle = 3 * (i / 3) + (j / 3)
  in
  match mreza.(i).(j) with
  | Some x -> None
  | None -> 
    let rec moznosti n acc =
      if n > 9 then Some (List.rev acc) else if (ni_v_stolpcu (mreza, j) n) && (ni_v_vrstici (mreza, i) n) && (ni_v_skatli (mreza, indeks_skatle) n) = true 
        then moznosti (n+1) (n :: acc) else moznosti (n+1) acc
    in
    moznosti 1 []
...
let rec resi mreza =
  let rec pretvori_vrstico vrstica acc =
    match vrstica with
    | [] -> List.rev acc
    | Some x :: xs -> pretvori_vrstico xs (x :: acc)
    | None :: _ -> failwith "Error"
  in
  let v_resitev mreza =
    Array.of_list (List.rev (Array.fold_left (fun acc vrstica -> pretvori_vrstico (Array.to_list vrstica) [] :: acc) [] mreza))
  in
  let rec prazna i j =
    if i > 8 then None
    else if j > 8 then prazna (i + 1) 0
    else
      match mreza.(i).(j) with
      | Some _ -> prazna i (j + 1)
      | None -> Some (i, j)
